...
assert.equal(cities.filter(c => c.sourceBackedScoreKeys.includes('rent')).length, 5);
assert.ok(!cities.find(c => c.slug === 'birmingham').signals.some(s => s.key === 'rent'));
console.log('Rent burden scoring and unsupported-city exclusion passed');
